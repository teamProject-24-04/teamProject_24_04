...
function MembersDetail() {
  ...
  useEffect(() => {
    async function fetchMemberDetail() {
      try {
        const response = await axios.post('/api/memberDetail', { id }); // Send ID in request body
  ...
  return (
    <>
      <Container>
        ...
        <div style={{ marginTop: '5%', display: 'flex' }}>
          ...
          <div style={{ marginRight: 'auto', display: 'flex', flexDirection: 'column' }}>
            <p style={{ margin: '0', marginBottom: '5px' }}>회원 레시피</p>
            <p style={{ margin: '0', marginTop: '15px' }}>{member.name}</p>
            <p style={{ margin: '0', fontSize: '10px' }}>{member.nickname}</p>
      ...
      <div style={{ fontSize: '12px', fontWeight: '500', marginTop: '20px' }}></div>
      <Button
        style={{
          width: '93%',
          marginTop: '20px',
